...
docs/
  ...
  CONFIGURATION.md                              (WP19) every audit.log.* property, one canonical table per module
.github/
  workflows/pages.yml                           (WP19) builds + publishes the GitHub Pages docs site
  pages-site/                                    (WP19) hand-authored hub/doc-viewer/Swagger-viewer pages that workflow assembles
...
```bash
# Aggregated Javadoc across every library module (WP19) - what .github/workflows/pages.yml
# publishes to the docs site. Install first so cross-module @link/@see references resolve.
mvn -f audit_log/pom.xml install -DskipTests
mvn -f audit_log/pom.xml org.apache.maven.plugins:maven-javadoc-plugin:3.8.0:aggregate
# -> audit_log/target/site/apidocs/index.html
```
...
not the last word on behavior. [`docs/CONFIGURATION.md`](docs/CONFIGURATION.md) is the canonical,
human-oriented version of this same table (WP19) - update both together when a property changes.
...
19. **The REST server's OpenAPI spec (WP19) is hand-authored, not generated by springdoc or any
    other runtime-introspection tool.** `audit-log-spring-boot-server` speaks Protobuf on the wire
    (`ProtobufHttpMessageConverter`, JSON via `protobuf-java-util`'s `JsonFormat`), not plain
    Jackson JSON - a tool that introspects the controllers/generated protobuf classes via Jackson
    (which is what springdoc does) would produce an inaccurate, noisy schema, since generated
    protobuf message classes expose internal getters (`getDescriptorForType`, `getSerializedSize`,
    `getAllFields`, ...) that Jackson would treat as real JSON properties but that never actually
    appear on the wire. Instead, `audit-log-server-openapi.yaml` is maintained by hand against
    `audit_event.proto` (the real source of truth for the wire shape) and bundled as a static
    resource, served by a small hand-written Swagger UI page (`swagger-ui/index.html`) rather than
    a `springdoc-openapi-starter-webmvc-ui` auto-configuration. The tradeoff: this spec can drift
    from the actual controllers if a change to `AuditIngestController`/`AuditQueryController` isn't
    also reflected here by hand - there is no build-time check enforcing they match. Update the
    `.yaml` in the same PR as any change to those controllers or to `audit_event.proto`.
...
- [`docs/CONFIGURATION.md`](docs/CONFIGURATION.md) (WP19) - every `audit.log.*` property across
  every module, in one canonical table per module, generated by hand from the `@ConfigurationProperties`
  classes themselves rather than duplicated ad hoc across other docs. Update this file (not just the
  class javadoc) whenever a property is added, renamed, or removed.
...
- The REST server's OpenAPI spec (WP19) -
  [`audit-log-spring-boot-server/src/main/resources/static/openapi/audit-log-server-openapi.yaml`](audit_log/audit-log-spring-boot-server/src/main/resources/static/openapi/audit-log-server-openapi.yaml),
  browsable at `/swagger-ui/index.html` on a running instance. **Hand-authored, not generated** -
  see "Conventions" #19 for why, and keep it in sync with `audit_event.proto` by hand whenever
  either changes.
- [`.github/workflows/pages.yml`](.github/workflows/pages.yml) (WP19) - builds and publishes the
  GitHub Pages docs site (aggregated Javadoc, the Swagger spec, every Markdown doc rendered) on
  every push to `main`. Site source lives in [`.github/pages-site/`](.github/pages-site).
...
- Each module's own `README.md` (`audit_log/audit-log-spring-boot-autoconfigure/README.md` is the
  ...
  `audit_log/audit-log-spring-boot-server/README.md` covers the REST endpoints, per-tenant
  authentication, and the Swagger UI specifically; `audit_log/audit-log-spring-boot-grpc-server/README.md`
  (WP18) covers the equivalent gRPC RPCs, and the REST/gRPC mutual-exclusion constraint,
  specifically; `audit_log/audit-log-java-client/README.md` and
