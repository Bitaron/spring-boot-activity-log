...
Aggregator repo for [`audit-log`](audit_log/README.md), a Spring Boot starter that records audit
trail entries for `@Audit`-annotated methods, and [`audit_log_usage_example`](audit_log_usage_example),
a runnable demo app that consumes it.
...
```bash
curl http://localhost:8080/test        # 200, records one audit_log row
curl http://localhost:8080/test/fail   # 500, records one audit_log row via @AfterThrowing
```
...
- **`audit_log`** - the starter. See [its README](audit_log/README.md) for configuration,
