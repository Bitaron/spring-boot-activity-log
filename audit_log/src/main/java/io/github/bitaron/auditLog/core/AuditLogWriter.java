...
/**
 * Performs the actual persistence of one {@code @Audit} invocation's audit record(s), inside its
 * own transaction ({@link Propagation#REQUIRES_NEW}) so that it neither depends on nor rolls
 * back with the caller's business transaction.
 * <p>
 * Kept as a separate bean from {@link AuditLogger} - rather than a method on it - so that
 * Spring's {@code @Transactional} proxy is actually invoked. Calling this method via
 * {@code this.persist(...)} from within the same class would silently bypass the proxy and the
...
public class AuditLogWriter {
    ...
    public void persist(Audit audit, AuditLogClientData clientData) {
        List<String> templateNameList = Arrays.stream(audit.templateNameList())
                .distinct()
                .toList();
